...
const getData = (params, callback) => {
  ...
  const previousStep = {};
  const unParsedCache = globalThis.rsc.getItem(cacheName);

  if (cache && unParsedCache) {
    try {
      const data = globalThis.rsc.parse(unParsedCache);
      const lastStep = data.renderedSteps[data.renderedSteps.length - 1];

      if (lastStep && lastStep.end) {
        globalThis.rsc.removeItem(cacheName);
      } else {
...
const setData = (cacheName, cachedData) => {
  const data = globalThis.rsc.parse(globalThis.rsc.stringify(cachedData));
  // clean components
  ...
  globalThis.rsc.setItem(cacheName, globalThis.rsc.stringify(data));
};
